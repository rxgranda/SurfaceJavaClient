...
public class TuioInputSource extends AbstractInputSource implements TuioListener {	
	...
	/** this is needed to track which events got fired as a blob down event already. */
	private Map<Long, Long> tuioBlobIDMap;
	/**
...
//		tuioClient.connect();
		...
		tuioBlobIDMap = new HashMap<Long, Long>();
...
//		logger.info("TUIO INPUT ADD FINGER - TUIO ID: " + sessionID);
		InputCursor c = new InputCursor(sessionID);
...
//		logger.info("Added TuioObj tuio object-> sessionID: " + session_id + " fiducialID: " + fiducial_id + " xpos:" + tuioObject.getX() + " ypos:" + tuioObject.getY() + " angle:" + angle + " x_speed:" + x_speed + " y_speed:" + y_speed + " r_speed:" + r_speed + " m_accel:" + m_accel + " r_accel:" + r_accel);
		...
		InputCursor c = new InputCursor((long)200);
...
//				logger.info("ERROR WHEN REMOVING TUIOOBJECT - TUIO ID: " + session_id + " - Cursor not in active cursor pool!");
	...
	public void addTuioBlob(TuioBlob tuioBlob) {
		...
		long session_id = tuioBlob.getSessionID();
		int blob_id = tuioBlob.getBlobID();
		float absoluteX =	tuioBlob.getX() * windowWidth;
		float abosulteY =	tuioBlob.getY() * windowHeight;
		float angle = tuioBlob.getContactAngle();
		float contwidth = tuioBlob.getContactWidth();
		float contheight = tuioBlob.getContactHeight();
		float contarea = tuioBlob.getContactArea();
		float x_speed=tuioBlob.getXSpeed();
		float y_speed=tuioBlob.getYSpeed();
		float r_speed=tuioBlob.getRotationSpeed();
		float r_accel = tuioBlob.getRotationAccel();
		float m_accel = tuioBlob.getMotionAccel();
		
		ArrayList<TuioPoint> puntos = tuioBlob.getPuntos();
		Iterator<TuioPoint> ite = puntos.iterator();
		while(ite.hasNext()){
			TuioPoint pttemp=ite.next();
			pttemp.xpos=pttemp.getX()*windowWidth;
			pttemp.ypos=pttemp.getY()*windowHeight;
			
			
		}
		
		//logger.info("Added TuioObj tuio blob-> sessionID: " + session_id + " fiducialID: " + blob_id + " xpos:" + absoluteX 
			//	+ " ypos:" + abosulteY + " angle:" + angle + " cwidth:" + contwidth + " contheight:" + contheight + " contarea:" + 
				//contarea + " pt1:" + p1.xpos + "," + p1.ypos);
		
		InputCursor c = new InputCursor((long)2000);

	
		
		MTBlobInputEvt blobEvt = new MTBlobInputEvt(this, absoluteX, abosulteY, MTFiducialInputEvt.INPUT_STARTED, c, blob_id, angle, x_speed, y_speed,r_speed, m_accel, r_accel,puntos);
		long cursorID = c.getId(); //TODO do implicitly somehow
		ActiveCursorPool.getInstance().putActiveCursor(cursorID, c);
		tuioBlobIDMap.put(session_id, cursorID);
		this.enqueueInputEvent(blobEvt);
	...
	public void removeTuioBlob(TuioBlob tuioBlob) {
		...
		Long cursorIDL = tuioBlobIDMap.get(session_id);
		if (cursorIDL != null){
			long cursorID = cursorIDL;
			InputCursor c = ActiveCursorPool.getInstance().getActiveCursorByID(cursorID);
			if (c != null){
				MTBlobInputEvt te;
				if (c.getCurrentEvent() != null)
					te = new MTBlobInputEvt(this, c.getCurrentEvent().getX(), c.getCurrentEvent().getY(), MTBlobInputEvt.INPUT_ENDED, c, blob_id);
				else
					te = new MTBlobInputEvt(this, 0,0, MTBlobInputEvt.INPUT_ENDED, c, blob_id);
				...
				tuioBlobIDMap.remove(session_id);
				ActiveCursorPool.getInstance().removeCursor(cursorID);
				this.enqueueInputEvent(te);
			}else{
//				logger.info("ERROR WHEN REMOVING TUIOOBJECT - TUIO ID: " + session_id + " - Cursor not in active cursor pool!");
			...
			logger.info("ERROR WHEN REMOVING TUIOBLOB - TUIO ID: " + session_id + " - Cursor not in tuioBlobIDMap!");
		}
	}
	...
	public void updateTuioBlob(TuioBlob tuioBlob) {
		...
		Long tuioID = tuioBlobIDMap.get(session_id);
		if (tuioID != null ){
			InputCursor c = ActiveCursorPool.getInstance().getActiveCursorByID(tuioID);
			if (c != null){
				MTBlobInputEvt blobEvt = new MTBlobInputEvt(this, absoluteX, abosulteY, MTFiducialInputEvt.INPUT_UPDATED, c, blob_id, angle, x_speed, y_speed,r_speed, m_accel, r_accel,puntos);
				this.enqueueInputEvent(blobEvt);
			}
		}
